...
构建为本地 `site.css`、`atheneum-pages.css`、`atheneum-scene.css` 与 `atheneum.js` 引用添加 `?v=` 内容版本，取对应文件 SHA-256 的前 12 位。文件变化后必须重新构建并发布生成的 HTML，让浏览器请求新版本。重复构建不改变版本；相对路径、其他查询参数及片段保留，外部网址不改写。
...
- 图中热区点击或使用原生按钮的 Enter／空格激活后，直接打开对应人物资料；热区的 `aria-haspopup="dialog"`、`aria-controls="atheneum-detail"` 及“阅读人物介绍”名称应与动作一致。摘要卡内的原生 `details.atheneum-chooser` 默认折叠，入口显示“选择哲学家”及数据中的人数。展开后，人名按钮的点击和左右方向键仅预览摘要；点击或按 Enter／空格选择后收起名单并把焦点返回 `summary`，再使用“阅读人物介绍”进入弹窗。左右方向键将名单中获得焦点的按钮滚动到可见位置。Escape、焦点移至选择器外或点击外部也会收起名单。
...
- 关闭按钮、Esc 和点击弹窗外背景均沿用原生弹窗关闭流程，关闭后回到最初触发元素。若该入口为图中热区且当前宽度不超过 680 px，则回到可见的选择器 `summary`，不能把焦点送进折叠名单。打开人物资料时也会收起名单。点击弹窗正文、工具条或内部留白不应关闭。
- 宽度不超过 680 px 时，图中热区设置 `tabindex="-1"` 与 `aria-hidden="true"`，保留指针点击，但不再作为键盘或辅助技术入口；可见的选择器 `summary` 提供替代入口。恢复桌面宽度时移除这两个属性。缩至小屏时若焦点正在热区上，应先移至该 `summary`；若焦点在打开的弹窗内，应保持其位置，直到关闭时再解析返回目标。
- 场景工具区的 `.atheneum-content-link` 是原生 `href="#atheneum-content"` 锚点，显示“浏览中心内容”／“Explore the center”，直接进入场景下方内容，不依赖人物弹窗或新增路由。
- `.atheneum-hint` 在生成模板及脚本中统一显示“选择人物，走近他的思想”／“Choose a philosopher to explore their ideas”，不随尺寸、景深开关或减弱动态偏好改变。

现有回归测试覆盖全部人物的按钮、记录、肖像引用与可访问名称关联、默认折叠的人物名单及其动态人数、双语提示，以及双语日期注释的传递和文本转义；视觉、热点位置和键盘实际操作仍需浏览器验收。交互修改后重点检查名单展开与收起、直接打开、连续切人后的回顶、三种关闭方式、原入口焦点恢复，以及弹窗打开和关闭时跨越 680 px 断点的行为。
